...
public class UsrMemberController extends Controller {
	...
	private void actionDoCheckPw(Rq rq) {
		String loginPw = rq.getParam("loginPw", "");
		String action = rq.getParam("action", "");
		...
		if(!loginPw.equals(rq.getLoginedMember().getLoginPw())) {
			rq.historyBack("입력하신 비밀번호가 일치하지 않습니다.");
			return;
		}
		if(action.length() == 0) {
			rq.replace("잘못된 접근입니다.", "../home/main");
			return;
		}
		
		
		if(action.equals("modifyMember")) {
			actionShowModify(rq);
		}
	...
	private void actionShowCheckPw(Rq rq) {
		String action = rq.getParam("action", "");
		if(action.length() == 0) {
			rq.replace("잘못된 접근입니다.", "../home/main");
			return;
		...
		rq.setAttr("action", action);
		rq.jsp("usr/member/checkPw");
	}
